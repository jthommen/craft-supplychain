...
contract Supplychain is Ownable, CraftsmanRole, AggregatorRole, RetailerRole, ConsumerRole {
  ...
  // General Actions
  // Get the craft or batch hash. craft hash = upc, batch hash = batch_no
  function getHash(
    string memory _name,
  ...
  // Batch Sales Actions
  // Put a batch with crafts up for sale
  ...
  // Finish producing a craft out of raw materials
  ...
  // Aggregator
  // Pick up of a sold batch at the craftsman by the aggregator
  function pickUpBatch(uint _batch_no) public onlyAggregator() batchOwnerOf(_batch_no) {
    batchRegistry.statePickedUp(_batch_no);
  }

  // Ships a batch to its owner
  function shipBatch(uint _batch_no, address _dest) public onlyAggregator() {
  ...
  // Take out the crafts from a batch in preparation for retail sales
  ...
  // Ships a sold craft to it's owner
  ...
  // Consumer Actions
  // Retrieve a batch price from a batch up for sale
  function getCraftPrice(uint _upc) public view returns(uint price) {
    return craftsForSaleMap[_upc];
  }

  // Buys a craft for sale
  ...
  // Receive a bought and shipped craft
